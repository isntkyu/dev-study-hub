...
파일을 읽을 때, 커널은 파일 서술자(숫자)를 반환하고 파일을 실행할 땐 커널에 파일서술자를 전달해야 한다.  
파일 서술자 덕에 프로세스는 파일에 대해 몰라도된다.

높은 파일 입출력 동시성을 위해 커널이 응용프로그램에 통지 하는 개념이 쓰인다.  
커널이 읽고 쓸 수 있는 파일 서술자를 알려준다.

#### 입출력 다중화 삼총사: select, poll, epoll

### mmap: 메모리 읽기와 쓰기 방식으로 파일 처리하기

가상메모리와 디스크 파일의 매핑.  
메모리를 읽고 쓰듯이 파일을 읽고 쓴다.  
큰 파일 처리에도 좋음. oom 안 일어난다. 가상메모리라서.  
32비트 시스템은 프로세스 주소공간이 4GB에 불과하다.  
mmap으로 동적라이브러리도 메모리에 매핑할 수 있다.
