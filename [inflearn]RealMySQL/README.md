...
1:N 조인을 _SKIP LOCKED_ 와 사용했을 때, 상위 테이블에 잠금이 걸리게 되어 의도한 대로 동작하지 않는다.

_OF_ 구문을 통해 상위 테이블의 잠금을 예외시켜 해결할 수 있다.
...
- 최적화된 바이너리 포맷으로 저장
- 중복 키-값은 마지막의 데이터로 저장
- JSON 데이터 내 키들을 정렬해서 저장
- 키는 중복저장되므로 적당한 길이로 사용하는 것을 권고
- json 타입의 부분업데이트를 사용하는게 성능이 매우 좋다.
  - 부분업데이트 기준
    - json_set(), json_replace(), json_remove() 함수만가능
    - 함수의 인자로 들어가는 컬럼과 변경컬럼이 일치해야함
    - 기존 값을 새로운 값으로 대체하는 형태여야함. 새로운 키-값이 추가되는 변경은 안됨.
    - 새로운 값은 기존 값보다 크기가 작거나 같아야함.
- 특정 키 값에 대해 함수기반 인덱싱 가능.
  - 배열인덱스 사용시 주의사항
    - member of(), json_contains(), json_opverlaps() 함수만 가능
    - 아직 버그가 종종 남아있는 기능이다.
  - 문자열인덱싱 시 따옴표 포함여부에 따라 결과가 달라지므로 주의(-> 오퍼레이터는 따옴표를 없앤다.)
  - 콜레이션 주의한다. CAST()함수에서 콜레이션 달라질 수 있어 대소문자 주의

### TEXT vs JSON

- json은 바이너리를 변환해서 전송하며 부분업데이트 가능 (json 유효성도 검사)
- TEXT타입도 json 함수 및 특정 키 값에 대한 인덱싱 가능
- 특정 키값만 주로 조회하고 변경한다면 json 타입 권장
...
- 사용예시: 로그성 데이터 테이블을 날짜별로 파티셔닝 하여 데이터 삭제가 아닌 파티션 드랍으로 처리
- 파티션 프루닝: 날짜 범위 파티셔닝을 할 때 MySQL이 조건에 해당하지 않는 파티션은 쿼리에서 제외함으로써 성능 이득을 가져가는 과정.
- 가장 많이 사용하는 RANGE & RANGE COLUMNS 타입

### 파티션 테이블 사용 제약

- 외래키, 공간 데이터, 전문검색 인덱스 사용 불가
- 모든 MYSQL 내장함수에 파티션 프루닝 지원하는 건 아님
  - TO_DAYS(), TO_SECONDS(), YEAR(), UNIX_TIMESTAMP()
- 모든 고유키에 파티셔닝 기준 컬럼이 포함되어있어야한다. (파티셔닝 테이블은 로컬인덱스)
- where 절에 파티셔닝 기준 컬럼에 대한 조건이 포함되어야 파티션 프루닝 적용된다.
- 값이 자주 변경되는 컬럼을 기준컬럼으로 사용하지 않는다.
- 예상치 못한 상황을 대비해 MAXVALUE 파티션 사용

> 파티션 프루닝이 적용되는 데이터타입, 내장함수, 쿼리 등을 잘 확인하고 설계
